...
import { saveTokenInLocalStorage, runLogoutTimer } from '../../../services/AuthService';
...
function Login (props) {	
	...
	async function onLogin(e) {
		...
		if (isForgotpassword) {
			try {
				const response = await axios.post("https://api.quantradeai.com/user/profiles/password/reset/", { identifier: email, new_password: password });
				if (response.data.status === 200) {
					let currentUrl = window.location.href;
					// Replace '/login' with '/dashboard'
					let updatedUrl = currentUrl.replace('/forgot-password', '/login');
			...
			try {
				const response = await axios.post("https://api.quantradeai.com/user/login/", { username: email, password: password });
				if (response.data.status === 200) {
									...
									<div id="sign-up" className="auth-form tab-pane fade show active  form-validation">
										<form onSubmit={onLogin}>
											<div className="text-center mb-4">
												<h3 className="text-center mb-2 text-dark">{isForgotpassword ? "Reset Password" : "Sign In"}</h3>
											...
											<div className="mb-3">
												<label htmlFor="exampleFormControlInput1" className="form-label required">UserName , Email or Mobile</label>
											...
											<div className="form-row d-flex justify-content-between mt-4 mb-2">
												<div className="mb-3">
													<div className="form-check custom-checkbox mb-0" style={isForgotpassword ? { display: "none", opacity: 0.6 } : {}}>
												...
												<div className="mb-4" style={isForgotpassword ? { display: "none", opacity: 0.6 } : {}}>
													<a href='forgot-password'  className="btn-link text-primary">Forgot Password?</a>
												</div>											
											</div>
											<button className="btn btn-block btn-primary">{isForgotpassword ? "Reset Password" : "Sign In"}</button>											
										</form>
										<div className="new-account mt-3 text-center">
											<p className="font-w500">Create an account? <Link className="text-primary" to="/page-register" >{buttontext}</Link></p>
										</div>
									</div>
