...
const Equity = (_data) => {
  ...
  const table_is_superuser = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      ...
      <Select
        value={filterBuySell}
        onChange={(e) => SetfilterBuySell(e.target.value)}
        label="Order Type"
        name="OrderType"
      >
      ...
      {loading ? (
        ...
        <>
          ...
          <Box mb={2} sx={{ backgroundColor: "white" }}>
            <Equity data={orders.net_positions} />
          </Box>
        </>
      )}
    </>
